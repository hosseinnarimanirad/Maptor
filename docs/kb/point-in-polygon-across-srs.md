# Point-in-polygon across SRS â€” move the points, not the polygon
...
millions of rows â€” transforming the polygons is far cheaper.
...
non-linear coordinate transform â€” geographic to projected, or between two
different projections â€” maps a straight segment to a **curve**. The redrawn
...
assigned to the **wrong polygon** â€” exactly the near-border points a count is
most sensitive to.
...
authoritative** â€” the SRS its geometry was authored in.

## Demonstration

![Point in polygon across SRS: reprojecting the polygon bends its edges; reprojecting the points is exact](../../src/Core/IRI.Maptor.Core.Spatial/images/point-in-polygon-srs.png)

The image (generated by
[`brand/social/generators/docs_point_in_polygon_srs.py`](https://github.com/hosseinnarimanirad/Maptor),
which also documents the simulated transform) shows three polygon regions with
long shared borders and 350 points:

- **Left card â€” reproject the polygon**: everything is pushed into the point
  layer's SRS. Vertices land true, but the solid chord edges drift off the
  dashed true boundary, and 25 points (the Ã— marks) are counted in the wrong
  region.
- **Right card â€” reproject the points**: the same data in the polygon's SRS;
...
answers are wrong â€” so a "the totals look fine" spot-check will not catch this
bug.

## Nuances and edge cases

- **If you truly must move the polygon** (for example you need it in the other
  SRS anyway), **densify** its edges first â€” insert intermediate vertices along
  each segment â€” and then reproject. This bounds the chord error but is still
...
- **Authoritativeness**: the result is correct *with respect to the SRS the
  ...
  form is the geodesic one â€” the rule generalizes to "test containment where
  the boundary definition is authoritative".
- **Performance**: transforming millions of points costs more than transforming
  a few polygons â€” that is the whole temptation. The cost is linear and
  parallelizable; correctness is worth it.

## Doing it in Maptor

Transform the point layer with the projection/transform APIs in
`IRI.Maptor.Core.SpatialReferenceSystem` (`MapProjects`, with SRIDs via
`SridHelper`), then count with the point-in-ring predicates in
`IRI.Maptor.Core.Spatial` (`TopologyUtility.IsPointInRing` or
`TopologyUtility.IsPointInRingUsingSignedAngles`) â€” in the polygon's SRS.

## Related

- [Analysis topics in IRI.Maptor.Core.Spatial](../../src/Core/IRI.Maptor.Core.Spatial/Analysis/README.md)
  â€” the point-in-polygon predicates themselves (ray casting vs winding number).
- [Map projections in IRI.Maptor.Core.SpatialReferenceSystem](../../src/Core/IRI.Maptor.Core.SpatialReferenceSystem/MapProjections/README.md)
  â€” the transforms that move the points.
