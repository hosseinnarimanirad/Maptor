...
- **Status:** partially implemented â€” **all bugs (B1â€“B9) and all perf items (P1â€“P5) plus S1
  done (2026-08-06)**; S2, S3, S4 still open
- **Area:** `src/Presentation/IRI.Maptor.Presentation.Wpf/Views/Map/MapViewer.xaml.cs` (~5,400 lines)
- **Companion doc:** `mapviewer-job-cancellation.md` (render-job cancellation redesign â€” items here
  deliberately do not repeat it)
- **Suggested order for what remains:** S2/S4 (small structure), then S3 (the job-cancellation
  redesign â€” its own doc). No bugs or perf items remain.

Line numbers were correct as of 2026-08-05 and are **stale** after the 2026-08-06 changes â€” treat
...
### B1. Wheel zoom has no zoom limit at all â€” âœ… DONE (2026-08-06)

> Fixed as part of the configurable-zoom work. `IsGoogleZoomLevelsEnabled` is no longer a dead
> branch: it became a computed shim over a new `IMapSettings.ZoomStepsPerGoogleLevel` (1â€“8, default
...
`IsGoogleZoomLevelsEnabled` branch â€” which is dead code: the setting
...
### B2. `ZoomToExtent` is `async void` and rethrows inside itself â€” âœ… DONE (2026-08-06)
...
rethrows from an `async void` continuation on the dispatcher â†’ **uncatchable process crash**.
...
### B3. Shared animation counters race between `Pan` and `ZoomToExtent` â€” âœ… DONE (2026-08-06)
...
(`++counter != counterValue`, `:3371`) â†’ the final `UpdateTileInfos()` + `Refresh` either never
fires (map left stale, callback dropped) or fires prematurely.

**Fix direction:** `ZoomToExtent` already has the correct pattern â€” `AnimateAsync` +
`Task.WhenAll` (`:3875-3884`). Port `Pan` to it and delete both counter fields.

### B4. A failed basemap tile render pops a blocking modal â€” âœ… DONE (2026-08-06)
...
(`:1676`). Any transient exception while rendering a basemap tile â€” once **per failing tile**,
including during shutdown or sign-out churn â€” throws a modal message box.

**Fix direction:** trace like the `AddNonTiledLayer` handler (`:1513-1519`) does.

## Bugs â€” medium

### B5. Unguarded `(LayerTag)` casts in every canvas scan â€” âœ… DONE (2026-08-06)
...
> file. `Find(TileInfo)` was left alone: it looks like the same bug but is already safe â€” the
...
subsequent `Clear`/`Refresh` â€” and `ClearOutOfExtent` runs inside `ExtentManager.Update`, so the
...
**Fix direction:** mechanical â€” `if (child.Tag is not LayerTag tag) continue;` at each site,
null-check `tag.Tile` in `ClearTileBorder`.

### B6. Re-`Register` leaks event handlers â€” âœ… DONE (2026-08-06)
...
`MouseUp`, `CurrentEditingPointChanged` (`:577-590`) with `+=` only â€” never unsubscribable. The
...
### B7. Complex-layer items measured before layout / fragile transform indexing â€” âœ… DONE (2026-08-06)
...
> X/Y in place â€” no assumption about the group's structure, and no per-move allocation. If an
...
`RenderTransformOrigin` compute from zeros â€” markers anchor wrong until repositioned.
`Item_OnPositionChanged` (`:2151`) hard-indexes `RenderTransform` `Children[2]` â€” valid only
...
### B8. `_unitDistance` cached forever â€” wrong after DPI change â€” âœ… DONE (2026-08-06)
...
### B9. Cancellation callbacks touch UI with `useSynchronizationContext: false` â€” âœ… DONE (2026-08-06)
...
> layer). The bezier token is **intentionally left `false`** and now says so in a comment â€” it only
...
## Improvements â€” performance

### P1. `CurrentExtent` recomputed inside loops â€” âœ… DONE (2026-08-06)
...
re-evaluated per canvas child in `ClearOutOfExtent` (up to 5Ã— per child, `:2326-2348`) and per
item in `AddComplexLayer`'s `Intersects` filter (`:1903`). Hoist to a local before each loop â€”
these run on every pan/zoom.

### P2. Linear `Children.Contains` scans per complex item â€” âœ… DONE (2026-08-06)

> Both `AddComplexLayer` batch sites (the main item loop and the `CollectionChanged â†’ Add`
> handler) now snapshot `mapView.Children` into a `HashSet<UIElement>` once per batch and pass
> it through `AddComplexLayerItem` â†’ `AddToCanvasWithAnimation`; newly added elements go into
> the set so later items in the batch see them. The set is per-batch/local â€” no long-lived
> mirror of the canvas to keep in sync â€” which is safe because both methods have no other
> callers. Chose this over the doc's "flag on `Locateable`" alternative to avoid a
> cross-project change and stale-flag risk. The `CollectionChanged â†’ Remove` branch keeps its
> `Contains` + `Remove` (small batches, and `Remove` is inherently O(n) anyway), as do the
> single-element `Contains` sites elsewhere in the file â€” they're not in loops.


`:1919`, `:2006`, `:2021` â†’ O(items Ã— children) per layer add. Track membership in a
`HashSet<UIElement>` or a flag on `Locateable`.

### P3. One 1-second animation clock per complex item â€” âœ… DONE (2026-08-06)
...
> opacity/scale anyway â€” a static add is the same end state, minus the transition.
>
...
> instead of `int.MaxValue`) â€” large complex batches (e.g. a long drawing item's vertex/edge
...
`AddToCanvasWithAnimation` (`:2024-2045`) starts a `DoubleAnimation` per item â€” hundreds of
concurrent clocks after a big layer add; each `AddFlash` adds three more with 5Ã— repeat
(`:2726-2749`). Animate a shared parent, or skip animation above an item-count threshold.

### P4. `UpdateZIndex` is O(layers Ã— children) â€” âœ… DONE (2026-08-06)
...
> semantics against any `Equals` override) and walks the canvas children once â€”
...
(`:625-633`) calls it per layer after a project load. Build one layerâ†’elements lookup per pass.

### P5. Basemap tiles render stretched between zoom levels â€” âœ… DONE (2026-08-06)
...
> every second notch land exactly on one. Surfaced in the the host application settings dialog and persisted.


Free-scale wheel zoom (see B1) means tiles rarely display at native resolution. Enabling the
dead `IsGoogleZoomLevelsEnabled` snapping â€” or exposing it as a user setting â€” gives crisper
basemaps and structurally eliminates the same-tile-list/empty-diff hazard class that caused the
2026-08 missing-tiles bug.

## Improvements â€” structure

### S1. Centralize zoom policy â€” âœ… DONE (2026-08-06)
...
> `ZoomToExtent` branches) â€” it snaps *and* clamps. `ClampScreenScale(double)` only bounds, and is
...
modern, cancellable, re-entrancy-guarded code â€” the model the navigation/render core should
converge on. Don't "improve" it while fixing the rest.

## Verification (when implementing)

1. Build `IRI.Maptor.Presentation.Wpf` + `your host application` (close any running the host application first â€” it
   locks output DLLs).
2. B1/S1: wheel-zoom to both extremes â€” scale stops at the configured levels; tiles stay sane.
3. B3: spam `PanTo` / zoom-to-extent operations back-to-back â€” map always ends refreshed,
   callbacks always fire exactly once.
4. B4: point the basemap at an unreachable URL â€” tiles fail silently to the not-found image, no
   message boxes.
5. B6: sign out and back in (re-`Register` path) â€” mouse-move handlers fire once (breakpoint),
