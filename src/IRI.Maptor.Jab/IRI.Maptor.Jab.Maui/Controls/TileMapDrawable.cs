...
internal sealed class TileMapDrawable : IDrawable
{
    ...
    public void Draw(ICanvas canvas, RectF dirtyRect)
    {
        ...
        if (urlFunc is null || Resolution <= 0)
        {
            return;
        }
        ...
        {
            var extent = tile.WebMercatorExtent;
        ...
        DrawLayers(canvas, viewXMin, viewYMax);

        DrawMarker(canvas, viewXMin, viewYMax);
    }

    private void DrawLayers(ICanvas canvas, double viewXMin, double viewYMax)
    {
        var layers = Layers;
        ...
        {
            if (!layer.IsVisible || layer.Parts.Count == 0)
            {
                continue;
            }
