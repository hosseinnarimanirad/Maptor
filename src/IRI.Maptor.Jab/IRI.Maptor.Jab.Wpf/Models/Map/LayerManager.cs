...
{
    public Action<IRI.Maptor.Jab.Wpf.Layers.BaseLayer>? RequestRefreshVisibility;
    ...
    public void Add(LegendViewModel legendViewModel, ILayer layer, double inverseMapScale)
    {
        ...
        else
        {
            ...
            {
                vl.RequestRefreshWhenDataLoaded = l => RequestRefreshVisibility?.Invoke(l as IRI.Maptor.Jab.Wpf.Layers.BaseLayer);
                _ = ds.LoadAsync(_loadCancellationToken);
            }
            else if (layer is RasterLayer rl && rl.DataSource is IDataSource rds && !rds.IsLoaded)
            {
                rl.RequestRefreshWhenDataLoaded = l => RequestRefreshVisibility?.Invoke(l as IRI.Maptor.Jab.Wpf.Layers.BaseLayer);
    ...
    private void RefreshLayerVisibility(object sender, EventArgs e) => RequestRefreshVisibility?.Invoke(sender as IRI.Maptor.Jab.Wpf.Layers.BaseLayer);

    private void Layer_OnLayerInitialized(object? sender, ILayer e) => RequestRefreshVisibility?.Invoke(sender as IRI.Maptor.Jab.Wpf.Layers.BaseLayer);
}
