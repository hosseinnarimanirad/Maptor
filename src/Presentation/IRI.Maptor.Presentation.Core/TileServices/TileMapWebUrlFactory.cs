...
using static IRI.Maptor.Presentation.Core.Properties.Resources;
...
public static class TileMapWebUrlFactory
{
    ...
    public static string GetServer(int min = 0, int max = 3)
    {
        ...
        return IRI.Maptor.Core.Common.Helpers.RandomHelper.Get(min, max + 1).ToString(System.Globalization.CultureInfo.InvariantCulture);
    }

    public static char GetServerCharacter(int min = 0, int max = 2)
    {
        //first bound is inclusive second bound is exclusive
        var random = IRI.Maptor.Core.Common.Helpers.RandomHelper.Get(min, max + 1);
