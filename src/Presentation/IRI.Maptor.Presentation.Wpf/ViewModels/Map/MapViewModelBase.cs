...
using IRI.Maptor.Core.Spatial.IO.Gpx;
...
using IRI.Maptor.Infrastructure.GdiPlus;
...
using IRI.Maptor.Presentation.Wpf.Models.Print;
...
using TileInfo = IRI.Maptor.Core.Spatial.Model.TileInfo;
using IRI.Maptor.Presentation.Wpf.Models.Settings;
...
using IRI.Maptor.Presentation.Core.Models.Project;
using IRI.Maptor.Presentation.Core.Data;
using IRI.Maptor.Presentation.Wpf.Services.Project;
...
public abstract class MapViewModelBase : ViewModelBase
{
    ...
    private void UpdateTilesServices()
    {
        ...
        // Swapping the url strategies only affects tiles fetched from here on, so without this the
        // tiles already on screen survive the switch and the new mode looks like it did nothing.
        // Null until the view registers, which makes the call from InitializeSettings a no-op.
    ...
    /// <summary>
    /// Asked after every identify click with the (already refreshed) results view model;
    /// the host shows its modeless results window or leaves the open one alone.
    /// </summary>
    public Action<IdentifyResultsViewModel>? RequestShowIdentifyResults;
    ...
    private IdentifyResultsViewModel? _identifyResults;
    /// <summary>
    /// State behind the identify results window. One instance per map: every identify click
    /// refreshes it in place, and the host's modeless window binds to it (see
    /// <see cref="RequestShowIdentifyResults"/>). Map coupling goes through the delegates wired
    /// in <see cref="CreateIdentifyResults"/>.
    /// </summary>
    public IdentifyResultsViewModel IdentifyResults => _identifyResults ??= CreateIdentifyResults();

    private IdentifyResultsViewModel CreateIdentifyResults()
    {
        var viewModel = new IdentifyResultsViewModel();

        // selection in the tree ↔ highlight overlay (flashes a handful of points, draws everything else)
        viewModel.RequestHighlightAsync = ShowHighlightedFeatures;

        viewModel.RequestFlash = features =>
        {
            var points = features.Where(f => f.GeometryType == GeometryType.Point)
                                 .Select(f => f.TheGeometry.AsPoint())
                                 .ToList();

            if (points.Count > 0 && points.Count == features.Count)
                FlashPoints(points);
            else
                _ = ShowHighlightedFeatures(features, null);
        };

        viewModel.RequestZoomTo = features =>
        {
            if (features.Count == 0)
                return;

            // a single feature goes through the point-aware zoom (a point's extent is degenerate)
            if (features.Count == 1)
            {
                Zoom(features[0].TheGeometry);
                return;
            }

            var extent = BoundingBox.GetMergedBoundingBox(features.Select(f => f.TheGeometry.GetBoundingBox()));

            ZoomToExtent(extent, false, true);
        };

        viewModel.RequestClearHighlight = () =>
        {
            ClearLayer(LayerType.Highlight, true);
            ClearLayer(LayerType.AnimatingItem, true);
        };
    ...
    /// <summary>
    /// Identify under <paramref name="point"/> and present the hits in the identify results
    /// window. Results are no longer pushed into <see cref="SelectedLayers"/> / the attribute
    /// table; that path is left to the legend's attribute-table command.
    /// </summary>
    public virtual async Task IdentifyAsync(Point point)
    {
        try
        {
            ...
            var results = new List<(VectorLayer Layer, FeatureSet<Point> Hits)>();

            foreach (var item in identify)
            {
                if (this.FindLayer(item.LayerId) is VectorLayer layer)
                    results.Add((layer, item));
            }

            // an empty list is a valid outcome: the window shows its "nothing here" state
            IdentifyResults.Update(point, results);

            RequestShowIdentifyResults?.Invoke(IdentifyResults);

            RemoveMapOptions();
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"IdentifyAsync failed: {ex}");
        }
        finally
        {
            IsBusy = false;
    ...
    {
        if (layer.DataSource is not IRI.Maptor.Core.Versioning.IVersionedEditTarget target)
            return true;
        ...
        var message = string.Format(
            IRI.Maptor.Presentation.Core.Localization.LocalizationManager.Instance["versioning_submit_confirm"],
            pendingCount,
            layer.LayerName);
    ...
    protected void TrySetCommands(ILayer layer)
    {
        ...
        if (layer is VectorLayer)
        {
            ...
            if (layer is IRI.Maptor.Presentation.Wpf.Layers.BaseLayer baseLayer)
            {
    ...
    public async Task OpenProjectAsync(object? owner)
    {
        ...
        var confirmed = await DialogService.ShowYesNoDialogAsync(IRI.Maptor.Presentation.Core.Properties.Resources.msg_project_confirmReplace, owner);
    ...
    public async Task OpenProjectFromAsync(string fileName, object? owner = null)
    {
        try
        {
            ...
            {
                await DialogService.ShowMessageAsync(IRI.Maptor.Presentation.Core.Properties.Resources.msg_project_unsupportedVersion, null, owner);
            ...
            {
                var message = string.Format(
                    IRI.Maptor.Presentation.Core.Properties.Resources.msg_project_skippedLayers,
