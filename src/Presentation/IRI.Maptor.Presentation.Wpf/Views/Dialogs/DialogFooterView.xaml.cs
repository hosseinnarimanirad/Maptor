...
public partial class DialogFooterView : UserControl
{
    ...
    /// <summary>
    /// Optional content at the footer's start edge, before the status line: typically the
    /// icon buttons of a dialog that acts on its own result (copy, add to drawing), so that
    /// every action shares the one footer row. Inherits the dialog's DataContext.
    /// </summary>
    ...
    public static readonly DependencyProperty LeadingContentProperty =
        DependencyProperty.Register(nameof(LeadingContent), typeof(object), typeof(DialogFooterView), new PropertyMetadata(null));
