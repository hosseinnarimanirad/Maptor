...
using IRI.Maptor.Core.Persistence.RasterDataSources;
using IRI.Maptor.Presentation.Wpf.Cartography;

using sb = IRI.Maptor.Core.Common.Primitives;
using IRI.Maptor.Presentation.Wpf.Controls.Controls;
...
using IRI.Maptor.Presentation.Wpf.Controls.Dialogs;
using IRI.Maptor.Presentation.Wpf.Controls.Identify;
...
public partial class MapViewer : NotifiableUserControl
{
                                ...
                                List<IriProvince93>? provinces = null)
    {
        ...
        presenter.RequestShowIdentifyResults = this.ShowIdentifyResults;
        ...
        // The base map layer is only ever created through RequestSetTileService, assigned above.
        // InitializeSettings assigns SelectedMapProvider before the view exists, and the resulting
        // SetTileService awaits a connectivity probe before invoking, so that invoke can land while
        // the delegate is still null and is then dropped with no retry — leaving the map with no
        // base map for the whole session. Re-apply here so the base map no longer depends on which
        // of the two wins. Going through the presenter (rather than invoking the delegate directly)
        // keeps the offline flag accurate, and RequestSetTileService is idempotent: it unsets the
        // existing base map tile layers before adding this one, so applying twice is harmless.
        if (presenter.SelectedMapProvider is not null && presenter.BaseMapSettings is not null)
        {
            // GetLocalFileName is optional all the way down to the TileServiceLayer ctor; only the
            // MapViewModelBase signature declares it non-nullable.
            _ = presenter.SetTileService(
                    presenter.SelectedMapProvider,
                    presenter.BaseMapSettings.BaseMapOpacity,
                    presenter.BaseMapSettings.GetLocalFileName!);
        }
    }
    ...
    private async Task AddTileServiceLayerAsync(TileServiceLayer layer, TileInfo tile)
    {
        try
        {
            ...
            try
            {
                fill = new ImageBrush(IRI.Maptor.Presentation.Wpf.Helpers.ImageUtility.CreateBitmapImage(geoImage.Image));
            }
            catch (Exception ex)
            {
                fill = new ImageBrush();
    ...
    public void Flash(List<IRI.Maptor.Core.Common.Primitives.Point> points)
    {
        //ClearAnimatingItems();
    ...
    public void Flash(IRI.Maptor.Core.Common.Primitives.Point mapPoint)
    {
        //ClearAnimatingItems();
        ClearLayer(LayerType.AnimatingItem, false);
    ...
    private void AddFlash(IRI.Maptor.Core.Common.Primitives.Point mapPoint, bool withAnimation = true)
    {
    ...
    public void Print()
    {
        //IRI.Maptor.Presentation.Wpf.Helpers.PrintHelper.Print(this.mapView);
        IRI.Maptor.Presentation.Wpf.Helpers.PrintHelper.Print(this);
    }
    ...
    private void ShowFeatureChangesDialog(IRI.Maptor.Core.Spatial.Primitives.Feature<sb.Point> feature, List<sb.Field>? fields)
    {
        var vm = new IRI.Maptor.Presentation.Wpf.ViewModels.Map.FeatureChangesViewModel(feature, fields);
    ...
    #region Identify results window

    private IdentifyResultsWindow? _identifyResultsWindow;

    // where the user left the window when they last closed it; null until then
    private Rect? _identifyResultsWindowBounds;

    /// <summary>
    /// Keeps one modeless results window per map. The view model is already refreshed when
    /// this is called, so an open window needs nothing; a closed one is (re)created. No
    /// Activate() on purpose: the user is clicking the map and focus should stay there.
    /// </summary>
    private void ShowIdentifyResults(IdentifyResultsViewModel viewModel)
    {
        if (_identifyResultsWindow is not null)
        {
            if (_identifyResultsWindow.WindowState == WindowState.Minimized)
                _identifyResultsWindow.WindowState = WindowState.Normal;

            return;
        }

        var window = new IdentifyResultsWindow
        {
            Owner = Window.GetWindow(this),
            DataContext = viewModel,
        };

        if (_identifyResultsWindowBounds is Rect bounds)
        {
        ...
        window.Closed += (s, e) =>
        {
            _identifyResultsWindowBounds = window.WindowState == WindowState.Normal
                                                ? new Rect(window.Left, window.Top, window.Width, window.Height)
                                                : window.RestoreBounds;

            _identifyResultsWindow = null;

            // drops the results and, through the view model's delegate, the map highlight
            viewModel.Clear();
        };

        _identifyResultsWindow = window;
    ...
    {
        //IRI.Maptor.Presentation.Core.Properties.Resources.mapPanel_currentPoint_copyCoordinate
        //leftToolTip: IRI.Maptor.Presentation.Core.Properties.Resources.mapPanel_edit_cancel,
        //rightToolTip: IRI.Maptor.Presentation.Core.Properties.Resources.mapPanel_edit_finish,

        var presenter = new MapOptionsViewModel(
            rightToolTip: IRI.Maptor.Presentation.Core.Properties.Resources.map_draw_finishDrawing,        //"تکمیل",
            leftToolTip: IRI.Maptor.Presentation.Core.Properties.Resources.map_draw_cancelDrawing,         // "لغو",
            middleToolTip: IRI.Maptor.Presentation.Core.Properties.Resources.map_draw_finishDrawingPart,   // "تکمیل تکه‌جاری",
    ...
    private async Task<Response<Geometry<sb.Point>>> Measure(DrawMode mode, Action action)
    {
        ...
        {
            Element = new IRI.Maptor.Presentation.Wpf.Controls.MapMarkers.LabelMarker(string.Empty)
        };
        ...
        onMoveForDrawAction = p =>
        {
            ...
            //var marker = (measureLayer.Items.First().Element as IRI.Maptor.Presentation.Wpf.Controls.MapMarkers.LabelMarker);
