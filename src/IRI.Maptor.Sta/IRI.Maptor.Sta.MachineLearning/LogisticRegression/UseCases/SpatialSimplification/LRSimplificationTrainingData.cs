...
public class LRSimplificationTrainingData<T> where T : IPoint, new()
{
        ...
        Func<T, T> toScreenMap = null)
    {
        ...
        for (int simplifiedIndex = 0; simplifiedIndex < simplifiedPoints.Count; simplifiedIndex++)
        {
            ...
            {
                if (IRI.Maptor.Sta.Spatial.Analysis.SpatialUtility.CalculateEuclideanDistance(currentPoint, originalPoints[originalIndex]) < IRI.Maptor.Sta.Spatial.Analysis.SpatialUtility.EpsilonDistance)
                {
