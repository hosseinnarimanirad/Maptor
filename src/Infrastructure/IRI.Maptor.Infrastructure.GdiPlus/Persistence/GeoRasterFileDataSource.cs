...
public class GeoRasterFileDataSource : RasterDataSource
{
    ...
    //{
    //    this.geoRaster = IRI.Maptor.Infrastructure.GdiPlus.WorldfileFormat.WorldfileManager.ReadWorldfile(imageFileName, srid);
    ...
    public override async Task<bool> LoadAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var geo = await IRI.Maptor.Infrastructure.GdiPlus.WorldfileFormat.WorldfileManager.ReadWorldfileAsync(_imageFileName, _srid);
    ...
    public GeoReferencedImage Get(BoundingBox boundingBox)//, Func<string, GeoReferencedImage> func)
    {
        ...
        //try
        //{
        //    var result = IRI.Maptor.Infrastructure.WorldfileFormat.WorldfileManager.ReadWorldfile(this.imageFileName);

        //    this.Extent = result.GeodeticWgs84BoundingBox.Transform(i => IRI.Maptor.Core.SpatialReferenceSystem.Projection.GeodeticToMercator(i));
