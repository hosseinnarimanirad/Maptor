...
public static class GeometryExtensions
{
    ...
    public static SqlGeometry AsSqlGeometry<T>(this Geometry<T> geometry) where T : IPoint, new()
    {
        ...
        {
            return IRI.Maptor.Infrastructure.SqlServerSpatialExtension.Helpers.SqlSpatialHelper.CreateEmptySqlGeometry(type, geometry.Srid);
        }
    ...
    private static void AddGeometryCollection<T>(SqlGeometryBuilder builder, Geometry<T> geometry) where T : IPoint, new()
    {
        ...
        foreach (var item in geometry.Geometries)
        {
            if (geometry.IsNullOrEmpty())
            {
                IRI.Maptor.Infrastructure.SqlServerSpatialExtension.Helpers.SqlSpatialHelper.AddEmptySqlGeometry(builder, item.Type);
            }
    ...
    public static SqlGeography AsSqlGeography<T>(this Geometry<T> geometry) where T : IPoint, new()
    {
        ...
        if (geometry.IsNullOrEmpty())
        {
            return IRI.Maptor.Infrastructure.SqlServerSpatialExtension.Helpers.SqlSpatialHelper.CreateEmptySqlGeography(type, geometry.Srid);
        }
    ...
    private static void AddGeographyCollection<T>(SqlGeographyBuilder builder, Geometry<T> geometry) where T : IPoint, new()
    {
        ...
        foreach (var item in geometry.Geometries)
        {
            if (geometry.IsNullOrEmpty())
            {
                IRI.Maptor.Infrastructure.SqlServerSpatialExtension.Helpers.SqlSpatialHelper.AddEmptySqlGeography(builder, item.Type);
            }
