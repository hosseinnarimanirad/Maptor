...
format. Ships in the [IRI.Maptor.Core.Spatial](../../README.md) package; WPF integration lives in
...
(`IRI.Maptor.Core.Spatial.IO.Dxf`); the WPF extension methods only convert visual parameters to
...
`IRI.Maptor.Presentation.Wpf` ships a ready-made import dialog, `DxfOpenDialogView`, exposed through the
dialog service:
