...
`Rectangle.cs` — the key struct. `RTree.cs`, `RTreeNode.cs` — the tree and its nodes. `SFCRTree.cs` — the space-filling-curve variant. All in namespace `IRI.Maptor.Core.Spatial.AdvancedStructures`.
