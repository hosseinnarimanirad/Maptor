...
The algorithm toolbox of `IRI.Maptor.Core.Spatial`: triangulation and Voronoi diagrams, convex hulls, line simplification, topology predicates, space-filling curves, and network building — all on plain geometry types, no UI dependencies.
...
The `Topology/` enums (`PointPolygonRelation`, `LineLineRelation`, `PointCircleRelation`, …) name the answers; the predicates live in `TopologyUtility` (namespace `IRI.Maptor.Core.Spatial.Helpers`). Point-in-polygon ships in both classic flavours:
...
Transform the point layer with the projections in [`IRI.Maptor.Core.SpatialReferenceSystem`](../../IRI.Maptor.Core.SpatialReferenceSystem/MapProjections/README.md), then count with the point-in-ring predicates above — in the polygon's SRS:
...
```csharp
using IRI.Maptor.Core.Spatial.Analysis.Network;
