...
public static class GoogleMapService
{
    ...
    public static async Task<Response<GoogleDirectionsResult>> GetDirectionAsync(Point start, Point end, string key)
    {
        try
        {
            ...
            return await IRI.Maptor.Core.Common.Helpers.HttpTransport.GetAsync<GoogleDirectionsResult>(url);
        }
        catch (Exception ex)
        {
    ...
    public static async Task<Response<GoogleDistanceMatrixResult>> GetDistanceMatrixAsync(List<Point> origins, List<Point> destinations, string key)
    {
        try
        {
            ...
            return await IRI.Maptor.Core.Common.Helpers.HttpTransport.GetAsync<GoogleDistanceMatrixResult>(url);
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine(ex.Message);
    ...
    public async static Task<Response<GooglePlacesResult>> SearchAsync(string query, string key)
    {
        ...
        return await IRI.Maptor.Core.Common.Helpers.HttpTransport.GetAsync<GooglePlacesResult>($"{googlePlacesApiUrl}query={query}&key={key}");
    }

    public static Response<GooglePlacesResult> Search(string query, string key)
    {
        return IRI.Maptor.Core.Common.Helpers.HttpTransport.Get<GooglePlacesResult>($"{googlePlacesApiUrl}query={query}&key={key}");
    }
