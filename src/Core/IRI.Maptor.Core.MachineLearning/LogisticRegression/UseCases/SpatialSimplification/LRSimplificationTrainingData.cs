...
public class LRSimplificationTrainingData<T> where T : IPoint, new()
{
        ...
        Func<T, T> toScreenMap = null)
    {
        ...
        for (int simplifiedIndex = 0; simplifiedIndex < simplifiedPoints.Count; simplifiedIndex++)
        {
            ...
            {
                if (IRI.Maptor.Core.Spatial.Analysis.SpatialUtility.GetEuclideanLength(currentPoint, originalPoints[originalIndex]) < IRI.Maptor.Core.Spatial.Analysis.SpatialUtility.EpsilonDistance)
                {
