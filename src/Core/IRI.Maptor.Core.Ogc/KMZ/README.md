...
A KMZ is a ZIP archive whose root document (`doc.kml`) travels together with the icons and images it references. The readers and writers here (namespace `IRI.Maptor.Core.Ogc.Kml`) build on `KmlReader` / `KmlWriter`, so everything KML supports — features, styles, folders — works inside a KMZ too.
